...
type ToStr<F> = F extends string | number ? `${F}` : ""

type Join<T extends any[], U extends string | number, S extends string | number = ""> = T extends [infer F, ...infer O] ? `${S}${ToStr<F>}${Join<O, U, U>}` : "";
