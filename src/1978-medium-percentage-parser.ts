...
type OperatorParser<A extends string> = A extends `${infer F}${infer O}` ? F extends Operator ? [F, O] : ['', A] : ['', A];

type UnitParser<A> = A extends `${infer F}%` ? [F, '%'] : [A, '']

type PercentageParser<A extends string> = OperatorParser<A> extends [infer P, infer O] ? [P, ...UnitParser<O>] : never;
