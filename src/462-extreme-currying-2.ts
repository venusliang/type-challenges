...
type Curry<T extends (...any: any[]) => any, P extends any[] = Parameters<T>> = T extends (...args: infer A) => infer R
    ? (A extends [x: infer X, ...o: infer O]
        ? (arg: X, ...args: Partial<O>) => Curry<(...args: O) => R> : R)
    : never;
